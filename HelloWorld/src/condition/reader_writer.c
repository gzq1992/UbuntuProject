...
void* get_th(void* arg)
{
	...
	for(;i <= 100; i++)
	{
		...
		//This mutex unlock int line 71 matches mutex lock inside pthread_cond_wait(&s->rc,&s->rm),instead of that in line 66.
		//Inside pthread_cond_wait(&s->rc,&s->rm),the mutex is unlocked before this thread is waked up.
		pthread_mutex_unlock(&s->rm);
